...
Let's imagine you've just moved into your first share-house. One month the electricity bill arrives. The bill is £40, and you and your 3 house-mates decide that you will split it evenly between all the people in the house. This means you will pay £10. By doing this, you've just used a function!

What are the inputs, output, and the rule for relating the inputs to the outputs?
...
And the rule that relates the **inputs** to the **outputs**? The rule is just to *take the total bill and divide it by the number of house mates*. 

### Functions in `R`

Functions in `R` are no different to our verbal example above. The only difference is that they're written done in some computer code. We won't be writing functions ourselves in this, but we will be using them.
...
- Finally, the function will produce some **output**. In the example below, the output is being saved in a **variable** called `each_person_share`. If we didn't assign the output to a variable the it would just write out at the console.

<img src="images/a_function.png" width="100%"></img>


Now that you know the parts of a function, take a look at the code example below. Run the code to see what happens! Notice on **line 2** the named of the output variable is written again so that contents of the variable is printed out. Try changing the values for the input labelled `number_of_people` and `}total_cost` and see how the output changes. The output always changes in accordance with the inputs and our rule (*divide the total cost by the number of people*).

```{r example_function, exercise=TRUE}
...
say_hello(a_letter_string = "Josh")
```

#### Numbers as inputs

In this example, we are calling the function called `plus 5`. It has one input called `a_number`, and that input must be a number. Notice that there are no `" "` around the input value. 
...
In all the example above the arguments had names For example, for the `describe_input()` function, the input was named `input_variable`. Most, but not all, functions have named inputs. The main function you'll come across that does not have named input is the `here()` function. 

When a function **does not** have named inputs then you just write input value in the `()` without including an input name of the `=`. Below is an example of using the `here()` function without an input label. 

```{r function_no_label, exercise=TRUE}
here("data")
```

In the examples above whenever we've used a function with **named inputs** we've always included the name and the `=`. But this isn't always necessary. However, leaving out the names of the input slots can be tricky, especially when we use functions that take multiple inputs. 

### Functions with multiple inputs  


For most the examples we've looked at we only gave one input to the function. However, if you look back at the `split_power_bill()` example you'd have seen an example of a function that takes multiple inputs. If we have a function that takes multiple inputs then we can specify the inputs in any order as long as we name the inputs. Try running the two examples below. Notice that the inputs in the two examples are specified in different order but the output in both are the same.
...
We can also leave the input names out and the function call will still produce an output. Run the example below to see and example. 

```{r order_example3, exercise=TRUE, exercise.cap="Example 3"}
split_power_bill(40,400)
```
When you leave the input names out then the inputs need to be specified in a specific order. This order is decided by the person that wrote the function. Looking at the output from Example 3 and compared it to the output from Example 1 and Example 2. Can you figure out what order the inputs need to be specified in if you leave the input names out?
...
**Packages** are add-ons that you can install to extended the functionality of `R`. **Packages** are groups of *functions* that have been *packaged* together in collections. Almost all the functions you'll use in `R` are part of a **package**.  
...
`R` comes with a special function from installing a package from **CRAN**. This function is called `install.packages()`. The `install.packages()` only takes one **argument**—the **name of the package** you want to install. 
...
Sometimes in this course you'll be asked to install a package that ins't on CRAN. This will usually happen when you're asked to install a package that has been developed by one of the lectures at **Sussex**. 
...
Once the **remotes** package has been installed, you can install packages using the `remotes::install_github()` command. You've probably never noticed a  `::` in a command before. But don't worry, it's [explained below](#)
...
**Important** When you load the **here** package it prints out a little message. Some other packages also print out little messages when you load them. But most packages don't show anything when they load. 

Here is an example of loading the **glue** package. Run the example and notice that it doesn't show any output...

```{r load_pakage2, exercise=TRUE}
library("glue")
```
...
When this happens, `R` can behave unpredictably because now that one function name could refer to two different functions. We'll take a look at this with an example. 
...
You'll see that `R` prints out a little warning messaging starting with the phrase `The following object is masked from ...`. This warning message lets you know that you have load 2 packages that both have a `saycolour()` function. 
...
This **masking** can be a problem, because you won't always remember what order you loaded our packages in. Sometimes you might not even realise that you've loaded two packages with overlapping function names! 

#### Functions **from** specific packages

**Masking** and function name conflicts can be very annoying! But there is a simple way to avoid them forever. This is to always specify the **package name** together with the **function name**. I will also use this style in the tutorials, because it will help to learn which functions come from which packages. 

To specify the **package name** you just write `packagename::` before the function call. When do you this, it doesn't matter what order you load the packages. You'll always get the function from the package you want. Take a look at the two example below to see it at work. 

In this example we'll load `blue` and then `red`, but we'll use the `saycolour()` function from `blue` first.
...
Although it's not strictly necessary to use the `package_name::function_name()` format when using functions, because `function_name()` will work just as well it is good practise to use the `package_name::function_name()` format. Why? 

One reason is that when you read your code back you'll know what packages you're using just in case you format. But another reason if that it makes it easier to spot errors. 
...
The first error message tells you the function isn't available. But how do you fix it?
...
<!-- in the worksheet include an example where there's a function call that refers to a missing package. They need to fix it by installing the package -->
